    ...
    for level in ['major', 'sub', 'specific']:
        ...
        for cluster_id, file_list in clusters.items():
            ...
            for file_path in file_list:
                # Convert embedding path to fingerprint path
...
I will provide you with hierarchical clusters of voice memo insights at three levels:
- MAJOR themes (broadest categories) 
- SUB themes (medium granularity)
- SPECIFIC topics (most granular)

For each cluster, I'll provide the raw essences from all voice memos in that cluster.

Your task: For each cluster, create:
1. **title**: 3-5 words capturing the core theme
2. **description**: 1-2 sentences explaining what this cluster represents
...
Remember: Return ONLY the JSON object with titles and descriptions. Make titles concise (3-5 words) and descriptions informative (1-2 sentences)."""
    ...
    try:
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                # Validate structure
                valid_levels = ['major', 'sub', 'specific']
